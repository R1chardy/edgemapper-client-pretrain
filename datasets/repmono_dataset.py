...
class RepMonoUnsupervisedDataset(NYUDataset):
    def __init__(self, dataset_path, val, transform, *args, **kwargs):
        ...
        self.width = kwargs.pop("width", 320)
        self.frame_idxs = kwargs.pop("frame_idxs", [0, -1, 1])
        self.num_scales = kwargs.pop("num_scales", 1)
    ...
    def preprocess(self, inputs, color_aug):
        ...
        for k in list(inputs):
            ...
            if "image" in k:
                n, im, i = k
                for i in range(self.num_scales):
                    inputs[(n, im, i)] = inputs[(n, im, i - 1)].resize((240,320))
        ...
        for k in list(inputs):
            ...
            if "image" in k:
                ...
                input = to_tensor(f)
                inputs[(n, im, i)] = F.interpolate(input.unsqueeze(0), size=(240, 320), mode='bilinear', align_corners=False).squeeze(0)
                
                result = to_tensor(color_aug(f))
                inputs[(n + "_aug", im, i)] = F.interpolate(result.unsqueeze(0), size=(240, 320), mode='bilinear', align_corners=False).squeeze(0)
    ...
    def __getitem__(self, index):
        ...
        for i in self.frame_idxs:
            ...
            #     logger.debug(f"rgb_path: {self.filenames}")
            #     logger.debug(f"rgb_path: {rgb_path}")
            #     logger.debug(f"new frame path {index}, {i}, {new_frame_path}")
            #     if ("image", 0, -1) in inputs:
            #         del inputs[("image", 0, -1)]
            #     if ("image", -1, -1) in inputs:
            #         del inputs[("image", -1, -1)]
            #     continue
            ...
            inputs[("image", i, -1)] = new_frame.resize((240,320))
